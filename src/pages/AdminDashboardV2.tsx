...
import { Plus, Trash2, Edit, AlertCircle, CheckCircle, Users } from 'lucide-react';
import { Admin } from '@/types';

export default function AdminDashboardV2() {
  const navigate = useNavigate();
  const { currentAdmin, getAllAdmins, addAdmin, updateAdmin, deleteAdmin } = useAuth();
  ...
  return (
    <MainLayout>
      <div className="space-y-8 animate-fade-in">
        ...
        {successMessage && (
          <div className="p-4 rounded-lg bg-green-50 border border-green-200 flex items-center gap-3 animate-fade-in">
                    ...
                    <Card key={admin.id} className="overflow-hidden">
                      <CardContent className="p-4 space-y-3">
                        <div className="flex justify-between items-start">
                          ...
                          <span className={`px-2 py-1 rounded text-xs font-semibold ${admin.role === 'super_admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                            {admin.role === 'super_admin' ? 'Super Admin' : 'Admin'}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between items-center pt-2 border-t">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${admin.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            {admin.isActive ? 'Active' : 'Inactive'}
                          </span>
